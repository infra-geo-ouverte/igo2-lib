...
## Légendes (UI et impression)

Le pipeline de légende est désormais aligné avec `layer.style` et accessible via un observable layer.style$.

Résolution de légende :

- pour `VectorLayer` et `VectorTileLayer`, la légende tente d'abord une résolution depuis le style actif de la couche;
- si aucune légende n'est calculable depuis ce style, un fallback datasource est utilisé.
