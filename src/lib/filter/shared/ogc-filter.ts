import { OgcFilter, IgoOgcFilterObject, WFSWriteGetFeatureOptions,
  AnyBaseOgcFilterOptions, OgcInterfaceFilterOptions } from './ogc-filter.interface';
...
public buildFilter(
  ...
  if (filters) {
      fieldNameGeometry = filters['geometryName'] !== undefined ?
      filters['geometryName'] : fieldNameGeometry;
  }
  if (extent && filters) {
...
if (filterObject instanceof Array) {
      ...
      if (filterObject.hasOwnProperty('logical')) {
            return this.createFilter({'operator': filterObject.logical,
...
private createFilter(filterOptions): OgcFilter {
  ...
  const wfs_singleChar = filterOptions.singleChar ? filterOptions.singleChar : '.';
  const wfs_escapeChar = filterOptions.escapeChar ? filterOptions.escapeChar : '!';
...
switch (operator) {
  ...
  case 'PropertyIsLike':
      return f.like(wfs_propertyName,
          wfs_pattern.replace(/[()_]/gi, wfs_singleChar), wfs_wildCard,
...
public defineInterfaceFilterSequence(
  ...
  geometryName,
  logical= '',
  level = -1): OgcInterfaceFilterOptions[] {
  if (filterObject instanceof Array) {
        filterObject.forEach(element => {
          this.filterSequence
          .concat(this.defineInterfaceFilterSequence(element, geometryName, logical, level));
        });
  } else {
        if (filterObject.hasOwnProperty('logical')) {
          level = level + 1;
          this.filterSequence
          .concat(this.defineInterfaceFilterSequence(
              filterObject['filters'], geometryName, filterObject['logical'], level));
        } else if (filterObject.hasOwnProperty('operator')) {
          this.filterSequence
          .push(this.addInterfaceFilter(filterObject, geometryName, level, logical));
        }
  }
return this.filterSequence;
}

public addInterfaceFilter(igoOgcFilterObject= {'operator': 'PropertyIsEqualTo'},
geometryName = undefined, level= 0, parentLogical= 'Or' ): OgcInterfaceFilterOptions {
  const f = { 'propertyName': '', 'operator': '', 'active': '', 'filterid': uuid(),
  'begin': '', 'end': '', 'lowerBoundary': '', 'upperBoundary': '', 'expression': '',
  'pattern': '', 'wildCard': '*', 'singleChar': '.', 'escapeChar': '!', 'matchCase': false,
  'igoSpatialSelector': '', 'geometryName': '', 'geometry': '', 'extent': '',
  'srsName': '', 'parentLogical': '', 'level': 0};

  return Object.assign(f,
      {'parentLogical': parentLogical, 'level': level, 'geometryName': geometryName },
      igoOgcFilterObject);
  }


public checkIgoFiltersProperties(filterObject: any, fieldNameGeometry, active= false) {
  const filter_array = [];
  if (filterObject instanceof Array) {
        filterObject.forEach(element => {
          filter_array.push(this.checkIgoFiltersProperties(element, fieldNameGeometry, active));
        });
  return filter_array;
      } else {
        if (filterObject.hasOwnProperty('logical')) {
          return Object.assign({},
              ...
              'filters': this.checkIgoFiltersProperties(filterObject['filters'],
              fieldNameGeometry, active)});
        } else if (filterObject.hasOwnProperty('operator')) {
...
private addFilterProperties(
  ...
  fieldNameGeometry, active= false) {
  const filterid = igoOgcFilterObject
  .hasOwnProperty('filterid') ? igoOgcFilterObject['filterid'] : uuid();
  const status = igoOgcFilterObject
  .hasOwnProperty('active') ? igoOgcFilterObject['active'] : active;

  return Object.assign({},
      {'filterid': filterid, 'active': status,
...
public rebuiltIgoOgcFilterObjectFromSequence(
  sequence: OgcInterfaceFilterOptions[]): IgoOgcFilterObject {
      ...
      sequence.forEach(uiFilter => {
        ...
        if (sequence.length === 1) {
          ...
          if (logicalArray.length === 1) {
            ...
            'least two operator in a logical (' + lastParentLogical + ')');
          } else {
