...
export class RoutingFormBindingDirective implements OnInit, AfterViewInit {
  ...
  constructor(@Self() component: RoutingFormComponent,
  ...
  ngAfterViewInit(): void {
    ...
    if (!storedStopsCoordinates && this.route && this.route.options.routingCoordKey) {
      this.route.queryParams.subscribe(params => {
      const routingParams = params[this.route.options.routingCoordKey as string];
      ...
      if (routingParams) {
         ...
         if (routingCoordUrl.length >= 2 ) {
          ...
          routingCoordUrl.forEach(coord => {
            ...
            this.component.stops.at(cnt).patchValue({stopCoordinates: stopCoordinatesFromURL});
            this.component.stops.at(cnt).patchValue({stopPoint: stopCoordinatesFromURL});
        ...
        for (let i = 0; i < storedStopsCoordinates.length; i++) {
          ...
          if (storedStopsCoordinates[i] instanceof Array) {
            ...
            this.component.stops.at(i).patchValue({stopCoordinates: storedStopsCoordinates[i]});
            this.component.stops.at(i).patchValue({stopPoint: storedStopsCoordinates[i]});
