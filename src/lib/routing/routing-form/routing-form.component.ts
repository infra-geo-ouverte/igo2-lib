...
import { RoutingService } from '../shared';
import { Message, LanguageService, MessageService } from '../../core';
import { Clipboard } from '../../utils';
import { RoutingFormService } from './routing-form.service';
import { ShareMapService } from '../../share-map';
import { FeatureDataSource } from '../../datasource/shared/datasources/feature-datasource';

@Component({
  selector: 'igo-routing-form',
  templateUrl: './routing-form.component.html',
  styleUrls: ['./routing-form.component.styl']
})
...
export class RoutingFormComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private routesResults: Routing[] | Message[];
  private activeRoute: Routing;
  private selectRoute;
  ...
  ngOnInit() {
    ...
    this.stopsForm = this.formBuilder.group({
      ...
      stops: this.formBuilder.array([this.createStop('start'), this.createStop('end')])
    });
  ...
  ngAfterViewInit(): void {
    ...
    this.selectRoute.on('select', evt => {
        if (this.focusOnStop === false) {
          const selectCoordinates = ol.proj.transform(
            evt['mapBrowserEvent'].coordinate, this.map.projection, this.projection)
          this.addStop();
    ...
    this.stopsForm.statusChanges.pipe(
      debounceTime(this._debounce)).subscribe(val => this.onFormChange())
    ...
    translateStop.on('translateend', evt => {
        ...
        const translationEndCoordinates =
        ol.proj.transform(evt.target.f.getGeometry().getCoordinates(),
        this.map.projection, this.projection);
        this.stops.at(p).patchValue({stopCoordinates: translationEndCoordinates});
 ...
 handleLocationProposals(coordinates: [number, number], stopIndex: number) {
  ...
  .map(res => res.map(f => f).subscribe(features => {
    ...
    this.stops.at(stopIndex).patchValue({stopProposals: groupedLocations});
    // TODO: Prefer another source?
       if (features[0]) {
         if ((features[0] as any).source === 'ICherche Québec') {
           ...
           this.stops.at(stopIndex).patchValue({stopPoint: features[featurePos].title});
           if ((features[featurePos] as any).geometry.type === 'Point') {
             this.stops.at(stopIndex)
             .patchValue({stopCoordinates: (features[featurePos] as any).geometry.coordinates});
            } else {
  ...
  moveStop(index, diff) {
    ...
    this.stops.at(index + diff).patchValue({routingText: this.routingText(index + diff)});
  ...
  formatInstruction (type, modifier, route, direction, stepPosition, exit, lastStep = false) {
    ...
    if (type === 'turn') {
        ...
        directiveFr = 'Continuer en direction ' + translatedDirection + ' sur ' + route;
        directiveEn = 'Head ' + translatedDirection + ' on ' + route;
        image = 'explore';
        cssClass = '';
    } else if (type === 'depart') {
        directiveFr = 'Aller en direction ' + translatedDirection + ' sur ' + route;
        directiveEn = 'Head ' + translatedDirection + ' on ' + route;
        image = 'explore';
        cssClass = '';
    } else if (type === 'arrive') {
        if (lastStep) {
          ...
          directiveEn = 'You have reached your destination' + coma + enAggregatedDirection;
        } else {
        ...
        directiveFr = 'À la fin de la route, tourner ' + translatedModifier + ' sur ' + route;
        directiveEn = 'At the end of the road, turn ' + translatedModifier + ' onto ' + route;
        ...
        directiveFr = 'Faire demi-tour et continuer en direction ' +
        translatedDirection + ' sur ' + route;
        directiveEn = 'Make u-turn and head ' + translatedDirection + ' on ' + route;
    } else  {
        directiveFr = '???';
        directiveEn = '???';
    }
  ...
  translateModifier(modifier) {
    if (modifier === 'uturn') {
       return this.languageService.translate.instant('igo.routing.uturn');
    } else if (modifier === 'sharp right') {
        return this.languageService.translate.instant('igo.routing.sharp right');
    } else if (modifier === 'right') {
      return this.languageService.translate.instant('igo.routing.right');
    } else if (modifier === 'slight right') {
      return this.languageService.translate.instant('igo.routing.slight right');
    } else if (modifier === 'sharp left') {
      return this.languageService.translate.instant('igo.routing.sharp left');
    } else if (modifier === 'left') {
      return this.languageService.translate.instant('igo.routing.left');
    } else if (modifier === 'slight left') {
      return this.languageService.translate.instant('igo.routing.slight left');
    } else if (modifier === 'straight') {
      return this.languageService.translate.instant('igo.routing.straight');
    } else {
        return modifier ;
    }
  };
  ...
  translateBearing(bearing) {
    if (bearing >= 337 || bearing < 23) {
        return this.languageService.translate.instant('igo.cardinalPoints.n');
    } else if (bearing < 67) {
        return this.languageService.translate.instant('igo.cardinalPoints.ne');
    } else if (bearing < 113) {
      return this.languageService.translate.instant('igo.cardinalPoints.e');
    } else if (bearing < 157) {
      return this.languageService.translate.instant('igo.cardinalPoints.se');
    } else if (bearing < 203) {
      return this.languageService.translate.instant('igo.cardinalPoints.s');
    } else if (bearing < 247) {
      return this.languageService.translate.instant('igo.cardinalPoints.sw');
    } else if (bearing < 293) {
      return this.languageService.translate.instant('igo.cardinalPoints.w');
    } else if (bearing < 337) {
      return this.languageService.translate.instant('igo.cardinalPoints.nw');
    } else {
        return ;
    }
  };
  ...
  showRouteSegmentGeometry(coordinates, zoomToExtent = false) {
    ...
    const geometry4326 = new ol.geom.LineString(coordinates)
    ...
    const geometry = new ol.geom.Point(lastPoint)
  ...
  getRoutes(stopsArrayCoordinates, moveToExtent = false) {
    ...
    if (routeResponse) {
      routeResponse.map(res => res.subscribe(
        (route) => {
  ...
  copyDirectionsToClipboard() {
    ...
    let wayPointList = '';
    const summary =
    this.languageService.translate.instant('igo.routingForm.summary') + ': \n' + indent +
    this.activeRoute.title + '\n' + indent +
    this.formatDistance(this.activeRoute.distance) + '\n' + indent +
    this.formatDuration(this.activeRoute.duration) + '\n\n' +
    this.languageService.translate.instant('igo.routingForm.stopsList') + ':\n';

    const url  = this.languageService.translate.instant('igo.routingForm.link') + ':\n' + indent +
    ...
    this.stops.value.forEach(stop => {
      ...
      if (stop.stopPoint !== stop.stopCoordinates) {
        ...
        coord = ' (' + [stop.stopCoordinates[1], stop.stopCoordinates[0]].join(',') + ')';
      } else {
        stopPoint = [stop.stopCoordinates[1], stop.stopCoordinates[0]].join(',');
      }

      wayPointList = wayPointList + indent + wayPointsCnt.toLocaleString() +
      '. ' + stopPoint + coord + '\n';
      wayPointsCnt++;
    });
    ...
    this.activeRoute.steps.forEach(step => {
      ...
      activeRouteDirective + indent + (localCnt + 1).toLocaleString() +
    ...
    if (successful) {
      ...
      const title = translate.instant('igo.routingForm.dialog.copyTitle');
      const msg = translate.instant('igo.routingForm.dialog.copyMsg');
  ...
  private handleTermChanged(term: string) {
    if (term !== undefined || term.length !== 0 ) {
      ...
      if (searchResponse) {
        searchResponse.map(res => res.subscribe(
          (features) =>  {
            (features as any).forEach(element => {
              if (searchProposals.filter(f => f.source === element.source).length === 0) {
            ...
            this.stops.at(this.currentStopIndex).patchValue({stopProposals: searchProposals})
          })
        )
      };
    };
  ...
  setTerm(term: string) {
    this.term = term;
    if (this.keyIsValid(term) && (term.length >= this.length || term.length === 0)) {
  ...
  keyup(i, event: KeyboardEvent) {
    ...
    this.setTerm(term);
    this.map.ol.un('singleclick', evt => {this.handleMapClick(evt as ol.MapBrowserEvent, i)})
  }
    ...
    if (proposal !== undefined ) {
      ...
      if (geom.type === 'Point') {
        ...
        if (geom.coordinates instanceof Array) {
          ...
          coordArray = geom.coordinates[Math.floor(geom.coordinates.length / 2)];
        } else {
      ...
      if (geomCoord !== undefined) {
        ...
        if (!ol.extent.intersects(proposalExtent, this.map.getExtent())) {
  ...
  private handleMapClick(event: ol.MapBrowserEvent, indexPos = undefined) {
    ...
    const clickCoordinates = ol.proj
    .transform(event.coordinate, this.map.projection, this.projection);
  ...
  public addStopOverlay(coordinates: [number, number], index: number) {
    ...
    if (routingText === 'start') {
      stopColor = 'green';
      stopText = this.languageService.translate.instant('igo.routingForm.start');
    } else if (routingText === 'end') {
      stopColor = 'red';
      stopText = this.languageService.translate.instant('igo.routingForm.end');
    } else  {
      stopColor = 'yellow';
      stopText = this.languageService.translate
      .instant('igo.routingForm.intermediate') +  ' #' +  index;
    }
