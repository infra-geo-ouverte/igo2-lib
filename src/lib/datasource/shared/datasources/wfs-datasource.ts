...
constructor(
  ...
  if (options['sourceFields'] === undefined) {
    ...
    .map(wfsCapabilities => options['wfsCapabilities'] = {
      ...
      f.values = this.dataSourceService.getValueFromWfsGetPropertyValues(options, f.name, 200, 0, 0)
    });
  }
  ...
  options.isOgcFilterable = options.isOgcFilterable === undefined ? true : options.isOgcFilterable;
  ...
  protected createOlSource(): ol.source.Vector {
      ...
      url: function(extent: ol.Extent, resolution, proj) {
        ...
        this.options['download'] = Object.assign({},
          ...
          this.options['download'], {'dynamicUrl': `${url}?${baseWfsQuery}&${wfsVersion}&${featureTypes}&${outputFormat}&${srsname}&${filterXML}&${maxFeatures}`});
        // tslint:disable-next-line:max-line-length
