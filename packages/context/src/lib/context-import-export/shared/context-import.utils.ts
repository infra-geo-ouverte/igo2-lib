...
export function addContextToContextList(
    ...
    Object.keys(contextService.contexts$.value.ours).forEach(
        key =>
          (contextService.enhancedContexts$.value.ours.push(contextService.contexts$.value.ours[key]))
    );
...
export function addImportedFeaturesToMap(olFeatures: OlFeature[], map: IgoMap, layerTitle: string): VectorLayer {
...
export function addImportedFeaturesStyledToMap(olFeatures: OlFeature[], map: IgoMap, layerTitle: string,
