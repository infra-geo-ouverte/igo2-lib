...
/** StyleFunction : (feature, resolution) => Style | Style[] | null | undefined */
...
export function isFlatStyleLike(value: unknown): value is FlatStyleLike {
  ...
  // Some OL APIs allow FlatStyleLike[]
  if (Array.isArray(value)) return value.every(isFlatStyleLike);

  if (!isRecord(value)) return false;

  return FLAT_STYLE_KEYS.some((k) =>
    Object.prototype.hasOwnProperty.call(value, k)
  );
}

/**
 * StyleLike generally means:
 * - Style instance
 * - Style[]
 * - StyleFunction
 */
...
/**
 * -----------------------------------------------------------------------------
 * Search/focus/selection styles
 * -----------------------------------------------------------------------------
 */

export function QueryResultsOlStyleFunction(
  viewController?: MapViewController,
  type?: 'focus' | 'selection'
): StyleLike {
  return SearchResultsOlStyleFunction(viewController, type);
}

export function SearchResultsOlStyleFunction(
  viewController?: MapViewController,
  type?: 'focus' | 'selection'
): (olFeature: OlFeature<OlGeometry>, resolution: number) => Style[] {
  return (olFeature: OlFeature<OlGeometry>) => {
    const { _mapTitle: mapTitle } = olFeature.getProperties();

    let imposedGeometry: OlGeometry | undefined;
    if (viewController) {
      const geom = olFeature.getGeometry();
      const geomExtent = geom?.getExtent() as [number, number, number, number];

      if (featuresAreTooDeepInView(viewController, geomExtent, 0.0025)) {
    ...
    switch (type) {
      case 'focus':
        return FocusOlStyle(mapTitle, imposedGeometry);
      case 'selection':
        return SelectionOlStyle(mapTitle, imposedGeometry);
      default:
        return BaseOlStyle(mapTitle, imposedGeometry);
    }
  };
}

export function BaseOlStyle(
  text?: string,
  imposedGeometry?: OlGeometry
): Style[] {
  const baseStyle = new Style({
    fill: new Fill({ color: 'rgba(0, 161, 222,0.15)' }),
    stroke: new Stroke({ width: 2, color: 'rgba(0, 161, 222,0.5)' })
  });

  if (imposedGeometry) baseStyle.setGeometry(imposedGeometry);
...
export function FocusOlStyle(
  ...
  const selectionStyle = new Style({
    fill: new Fill({ color: 'rgba(207, 222, 0, 0.15)' }),
    stroke: new Stroke({ width: 4, color: 'rgba(11, 220, 228, 0.5)' })
  });

  if (imposedGeometry) selectionStyle.setGeometry(imposedGeometry);
...
export function SelectionOlStyle(
  ...
  const selectionStyle = new Style({
    fill: new Fill({ color: 'rgba(0, 161, 222, 0.15)' }),
    stroke: new Stroke({ width: 4, color: 'rgba(0, 255, 255,0.5)' })
  });
 ...
 */
export function NearTransparentOlStyle(): Style {
  return new Style({
    fill: new Fill({ color: 'rgba(255, 255, 255, 0.01)' }),
    stroke: new Stroke({ width: 10, color: 'rgba(255, 255, 255, 0.01)' }),
    image: new Circle({
      stroke: new Stroke({ width: 10, color: 'rgba(255, 255, 255, 0.01)' }),
      fill: new Fill({ color: 'rgba(255, 255, 255, 0.01)' }),
      radius: 5
...
const SVG = (markerColor: string, markerOutlineColor: string) =>
  `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" height="36" width="36" viewBox="0 0 36 36">
  <path fill="${toString(ColorAsArray(markerColor))}" stroke="${toString(
    ColorAsArray(markerOutlineColor)
  )}" stroke-width="2"
...
export function MarkerOlStyle({
  text,
  markerColor = 'rgba(0, 161, 222, 1)',
  markerOutlineColor = 'rgba(255, 255, 255, 1)',
  imposedGeometry
}: MarkerOlStyleParams): Style {
  const svg = SVG(markerColor, markerOutlineColor);
  const dataImage = `data:image/svg+xml;utf8,${svg}`;
  ...
  if (imposedGeometry) style.setGeometry(imposedGeometry);
  return style;
}

export function RandomOlFlatStyle(): FlatStyleLike {
  const r = Math.floor(Math.random() * 255);
  const g = Math.floor(Math.random() * 255);
  const b = Math.floor(Math.random() * 255);

  return {
    'stroke-color': [r, g, b, 1],
    'stroke-width': 2,

    'fill-color': [r, g, b, 0.4],

    'circle-radius': 5,
    'circle-stroke-color': [r, g, b, 1],
    'circle-stroke-width': 2,
    'circle-fill-color': [r, g, b, 0.4],
...
export function ClusterOlStyleFunction(
  clusterParam: ClusterParam = {}
): (olFeature: OlFeature<OlGeometry>, resolution: number) => Style | Style[] {
  return (feature: OlFeature<OlGeometry>) => {
    ...
    if (size !== 1) {
      const radiusMin = 6;
      const clusterRadius =
        clusterParam.radiusCalc?.(size) ??
        Math.max(radiusMin, 5 * Math.log(size));
      ...
      return [
        new Style({
          image: new Circle({
            ...
            fill: new Fill({ color: 'rgba(24, 134, 45, 0.5)' })
          }),
          text: new Text({
            text: String(size),
