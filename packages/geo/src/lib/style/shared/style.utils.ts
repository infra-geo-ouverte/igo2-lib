import OlFeature, { type FeatureLike } from 'ol/Feature';
...
export function styleVariant(
  ...
  return (featureLike: FeatureLike) => {
    const { _mapTitle: mapTitle } = (
      featureLike as FeatureWithTitle
    ).getProperties();
    const imposedGeometry =
      featureLike instanceof OlFeature
...
export function clusterOlStyleFunction(
  ...
  return (feature: OlFeature<OlGeometry>) => {
    const size =
      (feature.get('features') as OlFeature<OlGeometry>[] | undefined)
        ?.length ?? 0;
...
function setGeometryIfNeeded<T extends Style>(
  style: T,
  geometry?: OlGeometry
): T {
  if (geometry) style.setGeometry(geometry);
  return style;
}

function createStyleGeometry(
  viewController?: MapViewController,
  feature?: OlFeature<OlGeometry>
): OlGeometry | undefined {
  if (!viewController || !feature) return undefined;
