...
export function createOverlayMarkerStyle(
  {text, opacity = 1, color = 'blue', outline = undefined}:
    {text?: string, opacity?: number, color?: string, outline?: string}  = {}
  ): olstyle.Style {
  let iconColor;
  let svg;
  switch (color) {
