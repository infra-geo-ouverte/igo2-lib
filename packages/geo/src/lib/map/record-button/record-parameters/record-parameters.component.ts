...
import { ImportService, handleFileImportSuccess } from '../../../import-export';
...
export class RecordParametersComponent implements OnInit {
  ...
  constructor(
    private dialogRef: MatDialogRef<RecordParametersComponent>,
      ...
      .subscribe((res) => {
        if (res === true) {
          ...
          this.geoDBService.update(
            'recordedTraces',
            this.trackFiles.length,
            this.trackFiles,
            InsertSourceInsertDBEnum.System,
            'recordedTraces' + this.trackFiles.length
          );
  ...
  displayParameters() {
    ...
    if (
      !geoMap ||
      !geoMap.position$.value ||
      !geoMap.position$.value.position
    ) {
  ...
  showWarning() {
    ...
    const dontShowAgain =
      this.storageService.get(
        'warningGPXRecord.dontShowAgain',
        StorageScope.LOCAL
      ) === 'true';
...
@Component({
  ...
  template: `
    ...
    <div mat-dialog-actions>
      <button
        mat-raised-button
        [mat-dialog-close]="{ dontShowAgain: data.dontShowAgain }"
      >
        OK
      </button>
    </div>
