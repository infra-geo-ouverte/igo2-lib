...
export class GeometryEditor {
  ...
  // Todo passer lar le layerservice?
  ...
  private addFeatureToMap(feature: Feature) {
    ...
    // TODO Modifier pour la logique de style par layer
    ...
    this.map.layerController.add(this.drawingLayer);
  ...
  private clearDrawings() {
    this.map.layerController.remove(this.drawingLayer);
  ...
  private createDrawControl(
    ...
    const control = new DrawControl({
      // TODO Pas sur.  geometryType: this.geometryType, avant c'etait  geometryType: undefined,
