...
export class FormFieldDatetimeComponent implements OnInit, OnDestroy {
   ...
   */
  readonly timePlaceholder = input<string>('Time');
  ...
  private pushInternalToExternal(): void {
    ...
    const datePart = this.dateTimeGroup.value.date;
    if (!datePart) {
    ...
    const sourceTime =
      this.dateTimeGroup.value.time ??
      this.toDateValue(this.formControl().value);
