...
export class PoiButtonComponent implements OnInit, OnDestroy {
            ...
            this.poiService.delete(poi.id).subscribe(
              () => {
                const title = translate.instant(
                  'igo.poiButton.dialog.deleteTitle'
                );
                const message = translate.instant(
                  'igo.poiButton.dialog.deleteMsg',
                  {
                    value: poi.title
                  }
                );
  ...
  createPoi() {
    ...
    const center: any = new ol.geom.Point(view.getCenter()).transform(
      proj,
      'EPSG:4326'
    );
          ...
          this.poiService.create(poi).subscribe(
            newPoi => {
              ...
              const titleD = translate.instant(
                'igo.poiButton.dialog.createTitle'
              );
              const message = translate.instant(
                'igo.poiButton.dialog.createMsg',
                {
                  value: poi.title
                }
              );
