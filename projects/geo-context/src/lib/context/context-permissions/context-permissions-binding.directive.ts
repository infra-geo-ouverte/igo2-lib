  ...
  onAddPermission(permission: ContextPermission) {
    ...
    this.contextService
      ...
      .subscribe(profils => {
        ...
        const message = translate.instant('igo.geoContext.permission.dialog.addMsg', {
          value: profil
        });
        const title = translate.instant('igo.geoContext.permission.dialog.addTitle');
  ...
  onRemovePermission(permission: ContextPermission) {
    ...
    this.contextService
      ...
      .subscribe(() => {
        ...
        const profil = permission.profil;
        const translate = this.languageService.translate;
        const message = translate.instant('igo.geoContext.permission.dialog.deleteMsg', {
          value: profil
        });
        const title = translate.instant('igo.geoContext.permission.dialog.deleteTitle');
        this.messageService.success(message, title);
      });
  }
  ...
  onScopeChanged(context: Context) {
    ...
    this.contextService.update(context.id, { scope: scope }).subscribe(() => {
      ...
      const message = translate.instant(
        'igo.geoContext.permission.dialog.scopeChangedMsg',
        {
          value: translate.instant('igo.geoContext.permission.scope.' + scope)
        }
      );
      const title = translate.instant(
        'igo.geoContext.permission.dialog.scopeChangedTitle'
      );
      this.messageService.success(message, title);
    });
  }
