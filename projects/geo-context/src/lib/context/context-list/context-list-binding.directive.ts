...
export class ContextListBindingDirective implements OnInit, OnDestroy {
  ...
  onSave(context: Context) {
    ...
    this.contextService.update(context.id, changes).subscribe(() => {
      ...
      const message = translate.instant(
        'igo.geoContext.context.dialog.saveMsg',
        {
          value: context.title
        }
      );
      const title = translate.instant(
        'igo.geoContext.context.dialog.saveTitle'
      );
  ...
  onFavorite(context: Context) {
    this.authService
      ...
      .subscribe(() => {
        ...
        const translate = this.languageService.translate;
        const message = translate.instant(
          'igo.geoContext.context.dialog.favoriteMsg',
          {
            value: context.title
          }
        );
        const title = translate.instant(
          'igo.geoContext.context.dialog.favoriteTitle'
        );
        this.messageService.success(message, title);
      });
  }
  ...
  onDelete(context: Context) {
    ...
    this.confirmDialogService
      .open(translate.instant('igo.geoContext.context.dialog.confirmDelete'))
      .subscribe(confirm => {
        if (confirm) {
          this.contextService.delete(context.id).subscribe(() => {
            const message = translate.instant(
              'igo.geoContext.context.dialog.deleteMsg',
              {
                value: context.title
              }
            );
            const title = translate.instant(
              'igo.geoContext.context.dialog.deleteTitle'
            );
  ...
  onClone(context: DetailedContext) {
    ...
    this.contextService.clone(context.id, properties).subscribe(() => {
      const translate = this.languageService.translate;
      const message = translate.instant(
        'igo.geoContext.context.dialog.cloneMsg',
        {
          value: context.title
        }
      );
      const title = translate.instant(
        'igo.geoContext.context.dialog.cloneTitle'
      );
      this.messageService.success(message, title);
    });
  }
