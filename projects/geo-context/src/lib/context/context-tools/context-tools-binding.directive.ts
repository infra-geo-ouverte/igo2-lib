...
export class ContextToolsBindingDirective implements OnInit, OnDestroy {
  ...
  onAddTool(tool: Tool) {
    ...
    this.contextService.addToolAssociation(contextId, tool.id).subscribe(() => {
      ...
      const message = translate.instant('igo.geoContext.tool.dialog.addMsg', {
        value: name
      });
      const title = translate.instant('igo.geoContext.tool.dialog.addTitle');
  ...
  onRemoveTool(tool: Tool) {
    ...
    this.contextService
      ...
      .subscribe(() => {
        const name = tool.title || tool.name;
        const translate = this.languageService.translate;
        const message = translate.instant('igo.geoContext.tool.dialog.removeMsg', {
          value: name
        });
        const title = translate.instant('igo.geoContext.tool.dialog.removeTitle');
        this.messageService.success(message, title);
      });
  }
